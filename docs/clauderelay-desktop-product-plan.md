# ClaudeRelay Desktop 产品方案

## 一、项目背景

### 1.1 现有 ClaudeRelay 的局限

ClaudeRelay 是一个极简的 Anthropic ↔ OpenAI 协议翻译代理，功能正确但配置全靠手改 JSON 文件，每次切换模型需要编辑配置文件后重启服务。
...
| [ClaudeRelay](https://github.com/ryanye981006-sudo/ClaudeRelay) | — | Node.js 原生 | 纯协议翻译代理：Anthropic ↔ OpenAI，双端口 CC + Codex |

**核心洞察**：

- cc-switch 是「配置管理器」 — 写配置文件，不做协议翻译
- claude-code-router (ccr) 是「智能路由器」 — 按场景自动调度，用户不直接控制用哪个模型
- ClaudeRelay 是「协议翻译器」 — 缺 GUI、缺模型管理
...
| | ccr | ClaudeRelay Desktop |
|---|---|---|
| 模型选择方式 | 自动调度（按场景路由） | **手动单选** |
| 用户心智 | "系统帮我选" | "我明确知道当前用哪个模型" |
| 切换机制 | `/model` 命令 + 自定义路由脚本 | **点击选中 → 写入配置** |
| 多工具支持 | 仅面向 CC | **CC + Codex 双分类** |
| 协议翻译 | 内置 transformers | **复用 ClaudeRelay 协议翻译引擎** |

## 二、产品定位

在现有 ClaudeRelay 协议翻译代理基础上，用 Electron 构建桌面客户端。用户通过 GUI 管理 API 源和模型，选择模型后自动写入 CC/Codex 配置文件，内嵌代理负责协议翻译。

**一句话**：现有 ClaudeRelay + Electron 壳 + 模型管理 GUI + 配置写入。
...
- 内嵌 ClaudeRelay，启动时自动拉起 :18888 和 :18889 两个端口
...
| 数据存储 | 单个 JSON 文件 | 零依赖，和 ClaudeRelay 风格一致 |
| 打包 | electron-builder | 跨平台安装包 |
| 开源协议 | MIT | 和现有 ClaudeRelay 保持一致 |

## 六、用户使用流程

```
1. 打开 ClaudeRelay Desktop
   → 代理自动启动 (:18888 + :18889)
